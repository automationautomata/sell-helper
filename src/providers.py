...
from core.infrastructure.marketplace import EbayAPI, MarketplaceAPI
...
class EbayProvider(Provider):
    ...
    @provide(scope=Scope.APP)
    def metadata_type(self) -> Type[EbayMetadata]:
        return Type[EbayMetadata]

    @provide(scope=Scope.APP)
    def marketplace_aspects_type(self) -> Type[MarketplaceAspects]:
        return Type[MarketplaceAspects]

    @provide(scope=Scope.APP)
    def search(
        self, perplexity_config: PerplexityConfig, ebay_config: EbayConfig
    ) -> SearchEngine:
        return PerplexityAndEbaySearch(perplexity_config, ebay_config)

    @provide(scope=Scope.APP)
    def merketplace(self, ebay_config: EbayConfig) -> MarketplaceAPI:
        return EbayAPI(ebay_config)
